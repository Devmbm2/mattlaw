...
class CustomSearchForm extends SearchForm
{
                            ...
                            case 'like':
                                if ($type == 'bool' && $field_value == 0) {
                                    ...
                                    if ($UnifiedSearch && strpos($field_value, ' ') !== false && strpos($db_field, $parms['db_field'][count($parms['db_field']) - 1]) !== false) {
                                        ...
                                        $where .= $this->concat_new($concat_table, $concat_fields) . " LIKE " . $this->seed->db->quoted($like_char .$field_value . $like_char);
                                        ...
                                        $where .= "REPLACE(" . $db_field .", ',', '')  like " . $this->seed->db->quoted(sql_like_string($field_value, $like_char));
                                    }
                                }
                                break;
                            case 'not in':
	...
	function concat_new($table, array $fields, $space = ' ')
    {
        if (empty($fields)) {
            return '';
        }
        $elems = array();
        $space = $this->seed->db->quoted($space);
        foreach ($fields as $field) {
            if (!empty($elems)) {
                $elems[] = $space;
            }
            $elems[] = $this->seed->db->convert("REPLACE($table.$field, ',', '')", 'IFNULL', array("''"));
        }
        $first = array_shift($elems);
        return "LTRIM(RTRIM(" . $this->seed->db->convert($first, 'CONCAT', $elems) . "))";
    }
}
