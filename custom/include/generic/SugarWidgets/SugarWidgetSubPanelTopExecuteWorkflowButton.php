...
class SugarWidgetSubPanelTopExecuteWorkflowButton  extends SugarWidgetSubPanelTopSelectButton
{
	...
	function display($widget_data, $additionalFormFields = NULL, $nonbutton = false)
	{
	    ...
	    $this->title     = 'Execute Workflows Manually';
        $this->accesskey = $this->getAccesskey();
        $this->value     = 'Execute Workflows';
		...
		$popup_request_data = array(
			...
			'call_back_function' => 'execute_workflow',
		...
		return ' <input type="button" name="' . $this->getWidgetId() . '" id="' . $this->getWidgetId() . '" class="button"' . "\n"
				. ' title="' . $this->title . '"'
			. ' value="' . $this->value . "\"\n"
			. " onclick='open_popup(\"AOW_WorkFlow\",600,400,\"$initial_filter\",true,true,$json_encoded_php_array,\"$popup_mode\",$create);' />\n";
	}
}
?>
