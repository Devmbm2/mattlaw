 ...
 * @api
 */
class CustomDashletGeneric extends Dashlet {
   /**
    ...
    function __construct($id, $options = null) {
		...
		if(isset($options)) {
            ...
            if(isset($options['listViewFilters'])) $this->listViewFilters = $options['listViewFilters'];
        }
    ...
    function process($lvsParams = array(), $id = null) {
        ...
        $this->addCustomFields();
        // apply filters
        if(isset($this->listViewFilters) && $this->listViewFilters) {
			$whereArray = $this->getListViewWhere();
		}elseif(isset($this->filters) || $this->myItemsOnly) {
   ...
   /**
	...
	function getListViewWhere() {
		global $db, $current_user, $log;
		$module = $this->seedBean->module_dir;

		require_once('include/export_utils.php');
		if(is_array($this->listViewFilters)){
			$contents = $this->listViewFilters;
		}else{
			$this->listViewFilters = addslashes($this->listViewFilters);
			$query = "SELECT * FROM `saved_search` WHERE deleted = 0 AND assigned_user_id = '{$current_user->id}' AND search_module = '{$module}' AND name LIKE '%{$this->listViewFilters}%';";
			$result = $db->query($query);  
			$row = $db->fetchByAssoc($result);
			$contents = unserialize(base64_decode($row['contents']));
		}
		$list_where =  generateSearchWhere($module, json_encode($contents));
		return $list_where['where'];
    }
