...
<script type="text/javascript" src='{sugar_getjspath file="custom/include/SugarFields/Fields/LocationAddress/SugarFieldAddress.js"}'></script>
{{assign var="map_key" value=$displayParams.key}}
{{assign var="key" value=$displayParams.key|upper}}
{{assign var="street" value=$displayParams.key|cat:'_of_incident_c'}}
...
<!-- <legend>{sugar_translate label='LBL_{{$key}}_OF_INCIDENT' module='{{$module}}'}</legend> -->
<table border="0" cellspacing="1" cellpadding="0" class="edit" width="100%">
<tr>
<td valign="top" id="{{$street}}_label" width='25%' scope='row' >
<label for="{{$street}}">{sugar_translate label='LBL_{{$key}}_OF_INCIDENT' module='{{$module}}'}:</label>
...
<input type="text" size="40px" id="{{$map_key}}_location_address" /><br>
...
{{else}}
<input type="text" size="40px" id="{{$map_key}}_location_address" /><br>
<textarea id="location_of_incident_c" name="location_of_incident_c" rows="{{$displayParams.rows|default:4}}" cols="{{$displayParams.cols|default:60}}" tabindex="{{$tabindex}}">{$fields.{{$street}}.value}</textarea>
{{/if}}
</td>
</tr>
