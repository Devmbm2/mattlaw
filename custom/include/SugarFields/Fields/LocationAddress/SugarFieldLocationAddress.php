...
class SugarFieldLocationAddress extends SugarFieldBase {
    ...
    function getDetailViewSmarty($parentFieldArray, $vardef, $displayParams, $tabindex) {
        ...
        if(!empty($valid['success'])) {
            $this->ss->assign("utgoogle_license","Ok");
        }
        else {
            $msg = "Invalid License";
            if($valid['result'])
                $msg = $valid['result'];
             $this->ss->assign("utgoogle_license",$msg);
        }
        $administration = new Administration();
        $administration->retrieveSettings();
        if(!empty($administration->settings['google_api_google_key']))
            $this->ss->assign("utgoogle_api",$administration->settings['google_api_google_key']);
        else
            $this->ss->assign("utgoogle_api","");
    ...
    function getEditViewSmarty($parentFieldArray, $vardef, $displayParams, $tabindex) {
        ...
        $administration = new Administration();
        $administration->retrieveSettings();
		/* print"<pre>";print_r($administration->settings); */
		$GLOBALS['log']->debug('test');
		$GLOBALS['log']->debug($administration->settings['google_api_google_key']);
        if(!empty($administration->settings['google_api_google_key']))
            $this->ss->assign("utgoogle_api",$administration->settings['google_api_google_key']);
        else
            $this->ss->assign("utgoogle_api","");
