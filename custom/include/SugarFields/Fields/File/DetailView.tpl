...
<a href="index.php?entryPoint=download&id={if isset($fields.{{$vardef.docType}}) && !empty($fields.{{$vardef.docType}}.value) && $fields.{{$vardef.docType}}.value == 'Local' && !empty($fields.{{$vardef.docUrl}}.value) }{$fields.id.value}{else}{$fields.{{$vardef.fileId}}.value}{/if}&type={{$vardef.linkModule}}" class="tabDetailViewDFLink" target='_blank'>{{sugarvar key='value'}}</a>
{{else}}
