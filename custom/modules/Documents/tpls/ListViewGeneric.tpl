	...
	$('#main_table').DataTable({
		...
		initComplete: function () {
			...
			this.api().columns([6]).every( function () {
				let column1 = this;
				let select1 = $('<select name = "email_documents" id = "email_documents" class="dt-search-select" style="width:160px;"><option value="">(No filter applied)</option><option value="1">Yes</option><option value="0">No</option></select>')
						.prependTo( $(column1.header()));

			});
			this.api().columns([7,8]).every( function () {
				var column = this;
				var select = $('<select name = "case_assigned_to" class="dt-search-select"><option value="no_filter">(No filter applied)</option></select>')
...
function caseSearch(){
	$('#case_other').on('change', function(){
		let search_value = $(this).find(":selected").val();
		let search_text = $(this).find(":selected").text();
		if (search_text == 'no_filter' || search_text == '') {
			$("#tbody_id").empty();
			$("#tbody_id").append(beforeAppend);
		}else{
			$.ajax({
				url:"index.php?module=Documents&action=index&return_module=Documents&return_action=DetailView&clear_query=true&filter_module=" + search_value,
				type: "post",
				...
				success:function(result){
					if($(result).find("#tbody_id").length == 1){
						...
						caseSearch();
						setTimeout(function(){
						$("#case_other").val(search_value);
						}, 3000);
						
						//alert($(result).find("#case_id").val());
						//console.log($(result).find("#case_id").val());
					}else{
						$("#tbody_id").empty();
	...
	$('#email_documents').on('change', function(){
		let search_value = $(this).find(":selected").val();
		let search_text = $(this).find(":selected").text();
		if (search_text == 'no_filter' || search_text == '') {
			$("#tbody_id").empty();
			...
			$.ajax({
				url:"index.php?module=Documents&action=index&return_module=Documents&return_action=DetailView&clear_query=true&email_documents=" + search_value,
				type: "post",
				data: {email_documents:search_value},
				success:function(result){
					if($(result).find("#tbody_id").length == 1){
						$("#main_table").remove();
						$("#main_table_wrapper").append($(result).find("#main_table"));
