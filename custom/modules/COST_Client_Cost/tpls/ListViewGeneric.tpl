...
{if $hideTable == false}
  ...
  <input type="text" name="search_u" id="search_u"  placeholder="Search with Name" size="180" maxlength="255" value="" title="">
  </div>
    ...
    "columnDefs": [
        ...
        initComplete: function () {
                ...
                this.api().columns([4]).every( function () {
					var column = this;
					var select_cases_v = $('<select  style="display:none;"   id = "select_cases" class="dt-search-select"><option value=""></option></select>')
						.prependTo( $(column.header()));

                        $.ajax({
                            url: 'index.php?module=COST_Client_Cost&action=get_cases_options',
                            type: 'POST',
                            ...
                            success: function(response){
                                ...
                                if(obj){
                                    $.each(obj, function(index, option) {
                                        select_cases_v.append($("<option/>").val(option.id).text(option.name));    
                                    });
                              }
                            }
                        });
                ...
                this.api().columns([5]).every( function () {
					var column = this;
					var select_payee_v = $('<select  style="display:none;"   id = "select_payee" class="dt-search-select"><option value=""></option></select>')
						.prependTo( $(column.header()));
                        $.ajax({
                            url: 'index.php?module=COST_Client_Cost&action=get_payee_options',
                            type: 'POST',
                            contentType: 'application/x-www-form-urlencoded',
                            dataType: 'text',
                            data: 'sugar_body_only=true',						
                            ...
                            success: function(response){
                              var obj = JSON.parse(response);
                                ...
                                if(obj){
                                    $.each(obj, function(index, option) {
                                        select_payee_v.append($("<option/>").val(option.id).text(option.name));
                                    });
                                } 
                            }
                        });
                ...
                this.api().columns([6]).every( function () {
					var column = this;
					var select_status = $('<select  style="display:none;"  id = "status_cost" class="dt-search-select"><option value=""></option></select>')
						.prependTo( $(column.header()));
						// ===Get language data and append in dropdown===
					    let case_status = SUGAR.language.languages.app_list_strings["invoice_status_dom"];
					    $.each(case_status, function (keys, values) {
						    $("#status_cost").append('<option value="'+ keys +'">'+ values +'</option>');
					    })
				});   
...
document.getElementById("select_cases").style.display="block";
document.getElementById("select_payee").style.display="block";
document.getElementById("status_cost").style.display="block";
$('#select_payee').css("width","150px");
$('#select_cases').css("width","150px");
$('#status_cost').css("width","150px");



let beforeAppend = $("#tbody_id").html();    
$(function(){     
$('#search_u').keyup(function(){
    search_filters();
...
$('#status_cost , #select_cases , #select_download ,  #select_payee' ).on('change' ,function(){
    search_filters();
            });
...
if($('#search_u').val() != ""){
  var searcheditem = $('#search_u').val();
}
  if($('#status_cost').val() != ""){
  var status_cost_v = $('#status_cost').val();
  }
  if($('#select_cases').val() != ""){
  var select_cases_v = $('#select_cases').val();
  }
  if($('#select_download').val() != ""){
  var select_download_v = $('#select_download').val();
  }
  if($('#select_payee').val() != ""){
  var select_payee_v = $('#select_payee').val();
  }
  var regex = /record=([\w-]+)/;
  if(typeof select_cases_v !== "undefined" && select_cases_v !== ""){
  var match = select_cases_v.match(regex);
  if (match) {
     select_cases_v = match[1];
  } 
  }
  if( typeof select_payee_v !== "undefined" && select_payee_v !== "") {
  var regex_payee = /record%3D([\w-]+)/;
  var match_payee = select_payee_v.match(regex_payee);
  if (match_payee) {
     select_payee_v = match_payee[1];
  }}
  if (searcheditem !== "" || select_cases_v !=="" || status_cost_v !==""  || status_payee_v !==""  || select_download_v !==""    ) {
  $.ajax({
  url:"index.php?module=COST_Client_Cost&action=index&return_module=COST_Client_Cost&return_action=DetailView&clear_query=true&search_data=searcheditem&status_cost_v=status_cost_v&select_cases_v=select_cases_v&select_payee_v=select_payee_v&select_download_v=select_download_v",
  type: "post",
  data:
 {search_data:searcheditem , status_cost_v:status_cost_v , select_cases_v:select_cases_v , select_payee_v:select_payee_v  , select_download_v:select_download_v }, 
