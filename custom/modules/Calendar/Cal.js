...
CAL.GR_update_focus = function(module, record) {
    if (record == "") {
        ...
        var callback = {
            success: function(o) {
                res = eval(o.responseText); /* SugarWidgetScheduler.update_time(); */
...
CAL.load_form = function(module_name, record, edit_all_recurrences, cal_event) {
    ...
    if (module_name == "Tasks") {
        ...
        $('#btn-view-events').unbind().click(function() {
            window.open('index.php?module=' + cal_event.module + '&action=DetailView&record=' + cal_event.record+'&redirect=1');
        });
        $('#btn-events-full-form').unbind().click(function() {
            window.open('index.php?module=' + cal_event.module + '&action=EditView&record=' + cal_event.record+'&redirect=1');
        });
    }
        ...
        var callback = {
            success: function(o) {
                ...
                if (res.access == 'yes') {
                    ...
                    eval(res.gr); /* SugarWidgetScheduler.update_time(); */
                    if (CAL.record_editable) {
                        CAL.enable_buttons();
                    }
...
CAL.update_vcal = function() {
    ...
    var callback = {
        success: function(result) {
            ...
            GLOBAL_REGISTRY.freebusy_adjusted[v] = SugarVCalClient.parseResults(result.responseText, true); /* SugarWidgetScheduler.update_time(); */
        }
    };
...
$($.fullCalendar).ready(function() {
   	SUGAR.util.doWhen(
		"typeof(calendar_items) != 'undefined'",
		makeCalendar
	);
	function makeCalendar(){
		...
		$.each(calendar_items, function(user_id, user_calendar_activities) {
			...
			$.each(user_calendar_activities, function(index, element) {
				...
				if (element.module_name == 'FP_events') {
					var travel_date_types = ['Deposition', 'Hearing', 'Intake', 'Mediation', 'Meeting', 'Trial'];
					var check = travel_date_types.includes(element.type_c);
					if(element.cancelled_reset_c == "Cancelled" || element.cancelled_reset_c == "Reset"){
						if(check == true){
							valueToPush["borderColor"] = '#ccc6c6;';
						}else{
							valueToPush["borderColor"] = '#808080';
						}
						valueToPush["backgroundColor"] = '#808080';
						valueToPush["textColor"] = '#FFFFFF';
					}else{
						if (element.type_c == 'Trial') {
							if(check == true){
								valueToPush["borderColor"] = '#ffcccb';
							}else{
								valueToPush["borderColor"] = '#FF0000';
							}
							valueToPush["backgroundColor"] = '#FF0000';
							valueToPush["textColor"] = '#FFFFFF';
						}else if (element.type_c == 'Vacation_Attorneys_Only') {
							if(check == true){
								valueToPush["borderColor"] = '#8a5a03';
							}else{
								valueToPush["borderColor"] = '#FFA500';
							}
							valueToPush["backgroundColor"] = '#FFA500';
							valueToPush["textColor"] = '#FFFFFF';
						}else if (element.type_c == 'Deadline') {
							if(check == true){
								valueToPush["borderColor"] = '#8a5a03';
							}else{
								valueToPush["borderColor"] = '#FFA500';
							}
							valueToPush["backgroundColor"] = '#6a0dad';
							valueToPush["textColor"] = '#FFFFFF';
						}
                        else if (element.status == 'travel') {
                            valueToPush["borderColor"] = '#3a87ad';
                            valueToPush["backgroundColor"] = '#3a87ad';
                            valueToPush["textColor"] = '#FFFFFF';
                        }
						
					}
				}   
                  if(global_colorUsersList !== null && undefined !== global_colorUsersList){
				if ( undefined !== element['assigned_user_id'] && undefined !== global_colorUsersList[element['assigned_user_id']]) {
					valueToPush["borderColor"] = '#' + global_colorUsersList[element['assigned_user_id']];
				} }
            ...
            select: function(date, jsEvent, view) {
                if (global_edit == true) {
                    ...
                    if (date.hasTime() == false) {
                        var date_end = date.add(1, 'hour').format(global_datetime_format);
                    }
...
function validateForm(){
	...
	var current_module = document.forms["CalendarEditView"].elements["current_module"].value;
	var required_fields = '';		
	if(current_module == 'FP_events'){
		var required_fields = {"name": "Name", "date_start_date": "Date", "date_end_date": "End Date", "cases_fp_events_1cases_ida": "Case" };		
	}else if(current_module == 'Calls'){
		var required_fields = {"name": "Name", "date_start_date": "Date"};		
	}
	var msg = '';
	for (var field in required_fields) {
		var value = document.forms["CalendarEditView"][field].value;
		if (value == "") {
		  msg += required_fields[field] +" must be filled out \n";
		  return_value = false;
		}
	}
	if(msg != ''){
