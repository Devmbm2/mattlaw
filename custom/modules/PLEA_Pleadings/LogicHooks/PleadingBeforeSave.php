...
class PleadingBeforeSave {
    public function QCProcessReview($bean, $event, $arguments) {
        global $db;
        $sql = "SELECT qc1_reason_for_fail_c,assistant_reason_for_fail_c,qc_review_remarks_c FROM plea_pleadings_cstm WHERE id_c = '{$bean->id}'";
        $result = $db->query($sql);
        ...
        // print_r($bean->qc_review_remarks_c);
        // die();
        
    }
    public function MarkDone($bean, $event, $arguments) {
        if($bean->assistant_reviewed_c == 'assistant_pass' && $bean->done_c == 1)
        {
            $bean->done_c = 1;
            // $bean->save();
        }
