 ...
 * @return array
 */
function getDisplayForFields($modulePath, $field, $reportModule)
{
    ...
    foreach($modulePathDisplay as &$module) {
        $module = isset($app_list_strings['aow_moduleList'][$module]) ? $app_list_strings['aow_moduleList'][$module] : (
...
function requestToUserParameterss()
{
    $params = array();
    if(isset($_REQUEST['parameter_id']) && $_REQUEST['parameter_id']) {
        foreach ($_REQUEST['parameter_id'] as $key => $parameterId) {
            ...
            // Fix for issue #1272 - aow_Report module cannot update Date type parameter.
...
function getConditionsAsParameterss($report, $override = array())
{
    if (empty($report)) {
        return array();
    }
    ...
    global $app_list_strings;
    $conditions = array();
    foreach ($report->get_linked_beans('aow_conditions', 'AOW_Conditions') as $key => $condition) {
        if (!$condition->parameter) {
            continue;
        }

        $path = unserialize(base64_decode($condition->module_path));
        $field_module = $report->flow_module;
        if ($path[0] != $report->flow_module) {
            foreach ($path as $rel) {
                if (empty($rel)) {
                    continue;
                }
        ...
        $disp = getDisplayForFields($path, $condition->field, $report->flow_module);
        $conditions[] = array(
            'id' => $condition->id,
            'operator' => $condition->operator,
            'operator_display' => $app_list_strings['aow_operator_list'][$condition->operator],
...
function getPeriodDates($date_time_period_list_selected)
{
    ...
    if ($sugar_config['aow']['quarters_begin']) {
        $q = calculateQuarterss($sugar_config['aow']['quarters_begin']);
    } else {
        $q = calculateQuarterss();
    }
 ...
 * @return DateTime
 */
function getPeriodEndDates($dateTimePeriodListSelected)
{
 ...
 */
function calculateQuarterss($offsetMonths = 0)
{
    // define quarters
    $q = array();
    $q['1'] = array();
