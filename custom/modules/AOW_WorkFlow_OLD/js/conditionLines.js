...
var ParenthesisHandler = {
    ...
    getParenthesisStartHtml: function(condition_id, logic_op, condition_order, _condln) {
        ...
        var html =
            ...
            '   <td>' +
            '       <input type="hidden" name="aow_conditions_parenthesis[' + ((_condln ? _condln : condln)) + ']" value="START">' +
...
function showConditionCurrentModuleFields(ln, value){
    ...
    if(report_module != '' && rel_field != ''){
        ...
        YAHOO.util.Connect.asyncRequest ("GET", "index.php?module=AOW_WorkFlow&action=getModuleFields&aow_module="+report_module+"&view=JSON&rel_field="+rel_field+"&aow_value="+value,callback);
...
function showConditionModuleField(ln, operator_value, type_value, field_value, overrideView, logic_value, condition_order, parenthesis){
    ...
    if(aow_field != ''){
		...
		$( ".start_paranthesis" ).nextUntil( ".end_paranthesis" ).attr( "style", "background: #f5f5f5;border: 1px solid #a5e8d6" );
        moduleFieldsPendingFinished++; YAHOO.util.Connect.asyncRequest ("GET", "index.php?module=AOW_WorkFlow&action=getModuleOperatorField&view="+overrideView+"&aow_module="+report_module+"&aow_fieldname="+aow_field+"&aow_newfieldname="+aow_operator_name+"&aow_value="+operator_value+"&rel_field="+rel_field,callback);
		moduleFieldsPendingFinished++; YAHOO.util.Connect.asyncRequest ("GET", "index.php?module=AOW_WorkFlow&action=getFieldTypeOptions&view="+overrideView+"&aow_module="+report_module+"&aow_fieldname="+aow_field+"&aow_newfieldname="+aow_field_type_name+"&aow_value="+type_value+"&rel_field="+rel_field,callback2);
        moduleFieldsPendingFinished++; YAHOO.util.Connect.asyncRequest ("GET", "index.php?module=AOW_WorkFlow&action=getModuleFieldType&view="+overrideView+"&aow_module="+report_module+"&aow_fieldname="+aow_field+"&aow_newfieldname="+aow_field_name+"&aow_value="+field_value+"&aow_type="+type_value+"&rel_field="+rel_field,callback3);
...
function showConditionModuleFieldType(ln, value, overrideView){
    ...
    var aow_field_name = "aow_conditions_value["+ln+"]";
	$( ".start_paranthesis" ).nextUntil( ".end_paranthesis" ).attr( "style", "background: #f5f5f5;border: 1px solid #a5e8d6" );
    YAHOO.util.Connect.asyncRequest ("GET", "index.php?module=AOW_WorkFlow&action=getModuleFieldType&view="+overrideView+"&aow_module="+report_module+"&aow_fieldname="+aow_field+"&aow_newfieldname="+aow_field_name+"&aow_value="+value+"&aow_type="+type_value+"&rel_field="+rel_field,callback);
...
function insertConditionLine(condition){
    ...
    if(view == 'EditView' && condition.parenthesis) {
        ...
        c.innerHTML += "<span style='width:178px;' id='aow_conditions_field_label" + condln + "' ></span>";
...
$(document).ready(function(){
  if($('#DefineStateOfWorkflow').children("option:selected").val()!='Opt_out'){
    $('#ReasonForOPTOut').hide();
    $("div[data-label='LBL_REASONFOROPTOUT']").hide();
  }

  $('#DefineStateOfWorkflow').change(function(){
    let searchParams = new URLSearchParams(window.location.search)
    var recordID=searchParams.get('record');
    if($(this).children("option:selected").val()=='Opt_out'){
      $('#ReasonForOPTOut').show();
      $("div[data-label='LBL_REASONFOROPTOUT']").show();

      $.ajax({
        type: 'POST',
        url: 'index.php?module=AOW_WorkFlow&action=UpdateWorkflowStatusField',
        data: {status:'Inactive',id:recordID},
      ...
      $.ajax({
        type: 'POST',
        url: 'index.php?module=AOW_WorkFlow&action=UpdateWorkflowStatusField',
        data: {status:'Active',id:recordID},
            success: function(data) {
              console.log(data);
            },
        });
