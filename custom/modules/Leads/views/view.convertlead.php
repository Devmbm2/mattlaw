...
class ViewConvertLead extends SugarView
{
        ...
        foreach($this->defs as $module => $vdef)
        {
            ...
            foreach($focus->field_defs as $field => $def)
            {
            	if(isset($vdef[$ev->view]['copyData']) && $vdef[$ev->view]['copyData'])
                {
	                ...
	                if ($module == "Cases" && $field == 'name')
	                {
			    //$lead_name = explode(" ", $this->focus->name,2);
	                    //$focus->name = $lead_name[0].", ".$lead_name[1];
			    $focus->type = $this->focus->case_type_c;
			    $focus->client_c = $this->focus->name;
			    if(!empty($this->focus->minor_incapacitated_name_c)){
				$focus->injured_person_c = $this->focus->minor_incapacitated_name_c;
				$contact_name = explode(" ", $this->focus->minor_incapacitated_name_c,2);
				$id = create_guid();
				$qry = "INSERT INTO contacts(id, first_name, last_name,deleted, date_modified) VALUES ('{$id}', '{$contact_name[0]}', '{$contact_name[1]}', 0, NOW())";
				$GLOBALS['db']->query($qry, true);
				$focus->contact_id2_c = $id;
			    }
			    $focus->representation_capacity_c = $this->focus->representative_capacity_c;
			    if(!empty($this->focus->first_name)) {
				$focus->name = $this->focus->last_name.", ".$this->focus->first_name;
			    }
			    else {
    ...
    protected function handleSave()
    {
        ...
        $saveBeanPriority = array('Contacts', 'Accounts', 'Cases');
        $tempBeans = array();
	    ...
	    //Special case code for Cases->Contacts
	    if ($bean->object_name == "Case" && empty($bean->contact_id1_c))
            {
		$GLOBALS['log']->fatal("Test convert lead");
                if (isset($beans['Contacts']))
                {
                    $bean->contact_id1_c = $beans['Contacts']->id;
                    $bean->client_c = $beans['Contacts']->name;
                }
                else if (!empty($selects['Contacts']))
                {
                    $bean->contact_id1_c = $selects['Contacts'];
                }
            }
        ...
        )
    {
    	global $beanList;
    	echo "<div><ul>";
	echo "<font size='5'>The Lead has been converted. Below are links to the new Case Record and the Contact Record.</font>";
        foreach($beans as $bean)
        {
            ...
            if ( $beanName == 'Contact' && !$this->new_contact ) {
                ...
                echo "<li><h5>" . translate("LBL_CREATED_NEW") . ' ' . $module_name . " -
                    <a href='index.php?module={$bean->module_dir}&action=DetailView&record={$bean->id}'>
                      <font color='red'> {$bean->get_summary_text()}</font>
                    </a></h5></li>";
            }
        }
        ...
        )
    {
		...
		if(!empty($beans['Cases'])){
				$sql = "SELECT * 
				        FROM linked_documents
				        WHERE deleted = 0 AND parent_id = '{$lead->id}' AND parent_type = 'Leads'; ";
				$result = $db->query($sql);
				while($row = $db->fetchByAssoc($result)){
					$query = "INSERT INTO documents_cases (id, document_id, case_id,deleted, date_modified)
					VALUES (UUID(), '{$row['document_id']}', '{$beans['Cases']->id}', 0, NOW())";
					$db->query($query, true);
				}
		}
