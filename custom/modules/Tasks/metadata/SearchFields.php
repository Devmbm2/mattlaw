...
$searchFields['Tasks'] = array (
  ...
  array (
    ...
    'subquery' => 'SELECT tasks.id
					FROM `tasks`
					INNER JOIN cases ON (cases.deleted = 0 AND cases.id = tasks.parent_id)
					INNER JOIN users ON (users.deleted = 0 AND cases.assigned_user_id = users.id)
					WHERE tasks.deleted = 0 AND tasks.parent_type = "Cases" AND CONCAT_WS(" ", users.first_name, users.last_name) IN (\'{0}\')',
    'db_field' => 
    array (
      0 => 'id',
    ),
  ...
  array (
    ...
    'subquery' => 'SELECT tasks.id
					...
					INNER JOIN cases ON (cases.deleted = 0 AND cases.id = tasks.parent_id)
					INNER JOIN users ON (users.deleted = 0 AND cases.assigned_user_id = users.id)
					WHERE tasks.deleted = 0 AND tasks.parent_type = "Cases" AND cases.status IN (\'{0}\') ',
    'db_field' => 
    array (
      0 => 'id',
    ),
