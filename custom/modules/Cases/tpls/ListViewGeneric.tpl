	...
	columnDefs : [
        ...
        initComplete: function () {
           this.api().columns().every( function () {
				...
				var case_status = SUGAR.language.get('app_list_strings', 'case_status_dom');
				this.api().columns([3]).every( function () {
					var column = this;
					var select = $('<select class="dt-search-select" id = "status" multiple ><option value="" >(No filter applied)</option></select>')
				...
				var complaint_type_list = SUGAR.language.get('app_list_strings', 'complaint_type_list');

				this.api().columns([4]).every( function () {
					var column = this;
					var select = $('<select class="dt-search-select" id = "type"><option value="">(No filter applied)</option></select>')
						.prependTo( $(column.header()));
									
						$.each(complaint_type_list, function( index, value ) {
								select.append( '<option value="'+ value +'">'+ value +'</option>' )
							});
				...
				this.api().columns([8]).every( function () {
					var column = this;
					var select = $('<select class="dt-search-select"><option value="">(No filter applied)</option></select>')
						.prependTo( $(column.header()))
						.on( 'change', function () {
						...
						/*$.ajax({
                            url: 'index.php?module=Cases&action=get_attorney_options',
                            type: 'POST',
                              ...
                              var obj = JSON.parse(response);
                                ...
                                if(obj){
                                    $.each(obj, function(index, option) {
                                        select.append($("<option/>").val(option.id).text(option.assistant_name));
                                    });
                                } 
                            }
                        });*/
				...
				this.api().columns([9]).every( function () {
					var column = this;
					var select = $('<select id="assistant_c" name = "attorney" class="dt-search-select" multiple><option value="">(No filter applied)</option></select>')
						.prependTo( $(column.header()))
						.on( 'change', function () {
							var val = $.fn.dataTable.util.escapeRegex(
								$(this).val()
						...
						$.ajax({
						url: `index.php?module=Cases&action=GetAllUsers`,
						type: "get",
						success: function (response) {
