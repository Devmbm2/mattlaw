...
class AOW_WorkFlow extends Basic {
	...
	function ACLAccess($view, $is_owner = 'not_set', $in_group = 'not_set'){
		if($view == 'edit' && $this->id == '8735ff52-225d-27d4-ceed-5db03dc1a822') return false;
...
function run_bean_flow(SugarBean &$bean , $id){
    ...
    function run_bean_flows(SugarBean &$bean){
        ...
        if(!isset($_REQUEST['module']) || $_REQUEST['module'] != 'Import'){
            ...
            //$query = "SELECT cases_aow_workflow_1aow_workflow_idb as id  FROM cases_aow_workflow_1_c  WHERE cases_aow_workflow_1cases_ida = '".$bean->id."'";
            // echo $query;die();
            //AND aow_workflow.status = 'Active' AND (aow_workflow.run_when = 'Always' OR aow_workflow.run_when = 'On_Save' OR aow_workflow.run_when = 'Create') AND aow_workflow.deleted = 0
            $result = $this->db->query($query, false);
            // print_r($result);die();
            $flow = new AOW_WorkFlow();
           // $checkboxIDs="";
            while (($row = $bean->db->fetchByAssoc($result)) != null){
                // echo "<pre>";print_r($flow ->retrieve($row['id']));echo "</pre>";die();
               // $checkboxIDs.='\''.$row['id'].'\''.',';
                $flow ->retrieve($row['id']);
                if($flow->check_valid_bean($bean))
                    $flow->run_actions($bean, true);
            ...
            // $query = "SELECT A.id FROM aow_workflow A LEFT JOIN cases_aow_workflow_1_c B ON A.id = B.cases_aow_workflow_1aow_workflow_idb WHERE B.id IS NULL AND A.flow_run_on='All_Records' AND A.status = 'Active'" ;
            //     // echo $query;
            // $result = $this->db->query($query, false);
            // // print_r($result);die();
            // while (($row = $bean->db->fetchByAssoc($result)) != null){
    ...
    function get_flow_beans(){
	  ...
	  public function queryWhereRepair($query_where)
    {
    ...
    function build_query_where(AOW_Condition $condition, $module, $query = array()){
        ...
        if(isset($app_list_strings['aow_sql_operator_list'][$condition->operator])){
            $where_set = false;
			$condition->logic_op = ($this->tiltLogicOp || $condition->condition_order == '0' ? '' : ($condition->logic_op ? $condition->logic_op . ' ' : ' AND '));
			$this->tiltLogicOp = false;
            ...
            if($condition->operator == 'is_null'){
                $query['where'][] = $condition->logic_op.'('.$field.' '.$app_list_strings['aow_sql_operator_list'][$condition->operator].' OR '.$field.' '.$app_list_strings['aow_sql_operator_list']['Equal_To']." '')";
                return $query;
            }
            ...
            switch($condition->value_type) {
                ...
                case 'Multi':
                    ...
                    if(!empty($multi_values)){
                        ...
                        $value .= ')';
                        $query['where'][] = $condition->logic_op.$value;
                    }
                    $where_set = true;
                    break;
                case 'SecurityGroup':
                    ...
                    $query['where'][] = $condition->logic_op.$sql;
                    $where_set = true;
                    break;
                case 'Value':
                default:
            ...
            if(!$where_set) $query['where'][] = $condition->logic_op.$field.' '.$app_list_strings['aow_sql_operator_list'][$condition->operator].' '.$value;
        } else {
			if ($condition->parenthesis) {
				if ($condition->parenthesis == 'START') {
					$query['where'][] = ($this->tiltLogicOp || $condition->condition_order == '0' ? '' : ($condition->logic_op ? $condition->logic_op . ' ' : 'AND ')) . '(';
					$this->tiltLogicOp = true;
				} else {
					$query['where'][] = ')';
					$this->tiltLogicOp = false;
				}
    ...
    function check_valid_bean(SugarBean &$bean){
        ...
        while ($row = $this->db->fetchByAssoc($result)) {
            ...
            if(!empty($path[0]) && $path[0] != $bean->module_dir){
    ...
    function run_actions(SugarBean &$bean, $in_save = false){
        ...
        //output : 8b3e2250-a0ac-5999-56b3-6322dd1fb3d2
        //ye workflow ki id ha jo workflow run ho ga cases k kisi bhi record k save hone par
        $processed->parent_id = $bean->id;
        //output : a2673f6e-7d5b-f4a7-37c0-624fd6c60fad
        //oper vali code line may vo ide a rahi ha jis record k ly workflow run ho raha ha
        $processed->parent_type = $bean->module_dir;
        //output : cases
        // oper vali code line ma ye module ka name a raha ha bs
        ...
        while ($row = $this->db->fetchByAssoc($result)) {
            ...
            //output : $this->multiple_runs -> 1

            //$action->action   output:CreateRecord
            ...
            if($this->multiple_runs || !$processed->db->getOne("select id from aow_processed_aow_actions where aow_processed_id = '".$processed->id."' AND aow_action_id = '".$action->id."' AND status = 'Complete'")){
                ...
                //echo "<pre>";print_r(file_exists('modules/AOW_Actions/actions/'.$action_name.'.php'));echo "</pre>";die();
                //output : 1
