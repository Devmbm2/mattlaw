...
{if $hideTable == false}
	<div class="list-view-rounded-corners">
		<table id = "main_table" cellpadding='0' cellspacing='0' border='0' class='list view table-responsive'>
			<input type="text" name="search_notes" id="search_note" class="form-control" placeholder="Search Notes by Subject">
	<thead>
...
// {*===========Searching in Note Type Dropdown Field in List View============*}
	$('#main_table').DataTable({
		...
		initComplete: function () {
			...
			this.api().columns([5]).every( function () {
				var column = this;
				var select = $('<select id = "note_type" class="dt-search-select"><option value="no_filter">(No filter applied)</option></select>')
						.prependTo( $(column.header()));
				// ===Get language data and append in dropdown===
				let note_types = SUGAR.language.languages.app_list_strings["note_type_c_list"];
				$.each(note_types, function (keys, values) {
					$("#note_type").append('<option value="'+ keys +'">'+ values +'</option>');
				})
				//    ==========Ajax Request===========
				$('#note_type').on('change', function(){
					...
					var dataTable = $('#main_table').DataTable({
						...
						"lengthChange": false,
						ajax: {
							"url": 'index.php?module=Notes&action=notesPurposeSearch',
							"type": 'POST',
							"datatype": "json",
							"data": {
								search_data:search
						...
						columns: [
							...
							{ "data": "id", render: function (data, type, row, meta) { return '<a  target="_blank" title="Edit" id="edit-' + row.id + '" href="index.php?module=Notes&amp;offset=2&amp;stamp=1641484446063359800&amp;return_module=Notes&amp;action=EditView&amp;record=' + row.id + '"><img src="themes/Honey/images/edit_inline.gif?v=V6Jf_6LIk4nKTRgtYTnxCA" border="0" alt="Edit"></a>'; } },
							{ "data": "date_entered",render: function (data, type, row, meta) { return '<b><a style="color: #e56455" target="_blank" href="?action=ajaxui#ajaxUILoc=index.php%3Fmodule%3DNotes%26offset%3D1%26stamp%3D1648544820038664200%26return_module%3DNotes%26action%3DDetailView%26record%3D' + row.id+ '">'+data+'</a></b>'; } },
							{ "data": "related_to",render: function (data, type, row, meta) { return '<b><a target="_blank" href="?action=ajaxui#ajaxUILoc=index.php%3Fmodule%3D'+row.module+'%26offset%3D1%26stamp%3D1648544820038664200%26return_module%3DCases%26action%3DDetailView%26record%3D' + row.parent_id+ '">'+data+'</a></b>'; } },
							{ "data": "subject",render: function (data, type, row, meta) { return '<b><a target="_blank" href="?action=ajaxui#ajaxUILoc=index.php%3Fmodule%3DNotes%26offset%3D1%26stamp%3D1648544820038664200%26return_module%3DNotes%26action%3DDetailView%26record%3D' + row.id+ '">'+data+'</a></b>'; } },
							{ "data": "note_type"},
							{ "data": "view_file",render: function (data, type, row, meta) { if(row.view_file){ return '<a href="index.php?entryPoint=download&amp;id='+row.id+'&amp;type=Notes" class="tabDetailViewDFLink" target="_blank" style="border-bottom: 0px;color: #e56455;font-weight: bold;">View File</a>' +
										                                                            '<a href="index.php?entryPoint=download&amp;id='+row.id+'&amp;type=Notes&amp;download=1" class="tabDetailViewDFLink" id="" name="" style="border-bottom: 0px;color: #e56455;font-weight: bold; display:block;" download="onclick" =="" check(this,this.name)="">' +
									                                                            	'<img src="https://img.icons8.com/metro/50/000000/download.png" width="22" height="22" style="display:block;margin:auto;"></a>'; }else{ return '<b></b>'; } } },
							{ "data": "created_by",render: function (data, type, row, meta) { return '<b><a target="_blank" href="?action=ajaxui#ajaxUILoc=index.php%3Fmodule%3DUsers%26offset%3D1%26stamp%3D1648544820038664200%26return_module%3DUsers%26action%3DDetailView%26record%3D' + row.user_id+ '">'+data+'</a></b>'; } },
							{ "data": "last_row", render: function (data, type, row, meta) { return '<span id="adspan_'+row.id+'" onclick="lvg_dtails('+"'"+row.id+"'"+')" style="position: relative;"><!--not_in_theme!--><img vertical-align="middle" class="info" border="0" alt="Additional Details" src="themes/Honey/images/info_inline.gif?v=V6Jf_6LIk4nKTRgtYTnxCA"></span>'; } },
						],
					});
				   }
				});
	...
	$('#search_note').on( 'keyup', function () {
		...
		if (search_value) {
			var dataTable = $('#main_table').DataTable({
				...
				bDestroy: true,
				ajax: {
					"url": 'index.php?module=Notes&action=liveSearch',
					"type": 'POST',
					"datatype": "json",
					"data": {
