<?php
$total_converted_leads_report = new total_converted_leads_report();
$total_converted_leads_report->generateTotal_converted_leads_ReportHTML('I');
class total_converted_leads_report{

	function generateTotal_converted_leads_ReportHTML($pdf_type = 'I'){
		...
		$sql = "SELECT * 
				...
				WHERE leads.deleted = 0 AND leads.status = 'Converted' AND DATE(leads.date_entered) BETWEEN DATE_SUB(CURDATE(), INTERVAL 7 DAY) AND CURDATE()
		...
		$header = '<table style="table-layout: fixed;height: 60px; width: 900px;border-spacing: 10px" border="0">
					...
					<tr border="0" style="height: 51px; color: white;">
						<td style="width: 232px; height: 51px;"><span style="color:black;font-size:15px;"><b>New Converted Leads Weekly Report</b></span></td>
		...
		ob_clean();
		if($pdf_type == 'I'){
			$pdf->Output("Total New Converted Leads Report.pdf", $pdf_type);				
		}else{
			if (!file_exists('cache/custom_reports')) {
				mkdir('cache/custom_reports', 0777, true);
			}
			$pdf->Output("cache/custom_reports/Total New Converted Leads Report.pdf", $pdf_type);				
		}
	}	
}