...
/*
        return array(
           ...
           translate('LBL_SAMPLELICENSEADDON','Administration').': '.translate('LBL_SAMPLELICENSEADDON_LICENSE_TITLE','Administration'),
           );*/
    ...
    public function display()
    {
        ...
        require_once('modules/'.$currentModule.'/license/htLoginTrackerOutfittersLicense.php');
        ...
        $key = htLoginTrackerOutfittersLicense::getKey($currentModule);
        if(!empty($key)) {
        ...
        if(isset($outfitters_config['manage_licensed_users']) && $outfitters_config['manage_licensed_users'] == true) {
            ...
            if(!empty($trimmed_last))
            {
                ...
                $validated = htLoginTrackerOutfittersLicense::doValidate($currentModule);
                $store = array(
                    'last_ran' => time(),
        ...
        if($validate_license !== true){
            $this->ss->assign('liscenceerrormessage','<p class="error">Login Tracker LicenseAddon is no longer active due to the following reason: '.$validate_license.' Users will have limited to no access until the issue has been addressed.</p>');
            $this->ss->assign('liscenceerror',true);
        }else{
            $this->ss->assign('liscenceerrormessage','');
            $this->ss->assign('liscenceerror',false);
        }
        $this->ss->display('modules/htLoginTrackerLicenseAddon/tpls/license.tpl');
    }//end of display
