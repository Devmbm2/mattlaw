...
$('#search_u').keyup(function(){
    console.log('search functions is working');
  let searcheditem = $('#search_u').val();
  $.ajax({
  url:"index.php?module=ht_formbuilder&action=index&return_module=ht_formbuilder&return_action=ListView&clear_query=true&search_data=searcheditem",
  type: "post",
  data:
 {search_data:searcheditem}, 
 success:function(result){
  if(result){
  $("#main_table").html("");
$("#main_table").append(
$(result).find("#main_table").html()
);
                        }
 }
  });
...
{if $hideTable == false}
  ...
  <input type="text" name="search_u" id="search_u"  placeholder="Search with Form Name" size="180" maxlength="255" value="" title="">
  </div>
    <table cellpadding='0' id="main_table" cellspacing='0' width='100%' border='0' class='list view table'>
        <thead>
