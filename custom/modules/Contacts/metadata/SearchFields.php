...
$searchFields['Contacts'] = array (
  ...
  array (
    ...
    'subquery' => 'SELECT contacts.id
					FROM `contacts`
					INNER JOIN contacts_cases ON (contacts_cases.deleted = 0 AND contacts_cases.contact_id = contacts.id)
					INNER JOIN cases ON (cases.deleted = 0 AND cases.id = contacts_cases.case_id)
					INNER JOIN users ON (users.deleted = 0 AND cases.assigned_user_id = users.id)
					WHERE contacts.deleted = 0 AND CONCAT_WS(" ", users.first_name, users.last_name) in ({0})',
    'db_field' => 
    array (
      0 => 'id',
    ),
  ...
  array (
    ...
    'subquery' => 'SELECT contacts.id
					...
					INNER JOIN contacts_cases ON (contacts_cases.deleted = 0 AND contacts_cases.contact_id = contacts.id)
					INNER JOIN cases ON (cases.deleted = 0 AND cases.id = contacts_cases.case_id)
					WHERE contacts.deleted = 0 AND cases.status  in ({0})',
    'db_field' => 
    array (
      0 => 'id',
    ),
