<?php
	...
	$dashlets_array = array(
		...
		array(
			...
			'dashlets' => array(
				array(
					'title' => 'Anita DiGiacomo Due Tasks',
					'listViewFilters' => array(
						'searchFormTab' => 'advanced_search',
						'query' => 'true',
		...
		array(
			...
			'dashlets' => array(
				array(
					'title' => 'Matt\'s Due Tasks with Case Status "Lit" or "Appeal"',
					'listViewFilters' => array(
						'searchFormTab' => 'advanced_search',
						'query' => 'true',
		...
		array(
			...
			'dashlets' => array(
				array(
					'title' => 'Matt\'s Due Tasks with Case Status "Pre"',
					'listViewFilters' => array(
						'searchFormTab' => 'advanced_search',
						'query' => 'true',
						'assigned_lawyer_cases_advanced' => array('Matthew D. Powell'),
	...
	foreach($dashlets_array AS $dashlets_data){
		...
		foreach($dashlets_data['users_list'] AS $id){
			...
			foreach($existingPages as $pid=>$page) {
				...
				if($page['pageTitle'] == 'HONEY DASHBOARD'){
					unset($existingPages[$pid]);
				}
			}
			if($notfound) {
