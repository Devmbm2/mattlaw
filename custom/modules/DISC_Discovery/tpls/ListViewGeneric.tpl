		...
		{* if $rowData.DONE==0 *}
			...
			<tr height='20' class='{$_rowColor}S1'>
				...
				{foreach from=$displayColumns key=col item=params}
                    ...
                    {$displayColumns[type]}
				    {strip}
					<td {if $col == 'STATUS'}     style = "padding-left: 40px; !important;" {/if} {if $scope_row} 
					{elseif $col=='NUMBER_OF_DAY_C'}
							{if $rowData.NUMBER_OF_DAY_C}
						...
						{else}
							{if $col=='NUMBER_OF_DAY_C'}
								{if $rowData.NUMBER_OF_DAY_C}
									{if $rowData.NUMBER_OF_DAY_C<0}

									<span class="counter">
									{$rowData.NUMBER_OF_DAY_C|abs}
									</span>
									{elseif $rowData.NUMBER_OF_DAY_C>0}
									<span class="counterGreen">
										{sugar_field parentFieldArray=$rowData vardef=$params displayType=ListView field=$col}
									</span>
									{/if}
								{else}
									{if $rowData.NUMBER_OF_DAY_C=='0'}
									<span class="counterOrange">
										{$rowData.NUMBER_OF_DAY_C}
									</span>
									{/if}
								{/if}
							{else}
	...
	$('#main_table').DataTable({
		...
		initComplete: function () {
			...
			this.api().columns([7]).every( function () {
				var column = this;
				var select = $('<select name = "case_assigned_to" class="dt-search-select"><option value="">(No filter applied)</option></select>')
