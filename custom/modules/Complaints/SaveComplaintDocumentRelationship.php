				...
				for( $i = 0; $i < $zip->numFiles; $i++ ){ 
					...
					if(isset($stat)){
						...
						if(!empty($pathBySlash[1]) && !empty($pathBySlash[2]) && !empty($pathBySlash[3])){
							$complaint_bean = BeanFactory::getBean('Complaints')->retrieve_by_string_fields(array('name'=>$pathBySlash[1])); 
							if($pathBySlash[2] == 'AAA'){
								$id = uploadDocument($complaint_bean, $pathBySlash[3], $zip, $stat);
...
function uploadDocument(&$parent_bean, $NoteFileName, &$zip, $fil_zip_address){
	...
	$revision->save();
	$id = create_guid();
	$sql = "INSERT INTO documents_complaints (id, date_modified, deleted, document_id, complaint_id) VALUES ('{$id}', NOW(), 0, '{$DocumentId}','{$parent_bean->id}' )";
	$db->query($sql);
