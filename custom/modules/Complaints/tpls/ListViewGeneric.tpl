...
{if $hideTable == false}
	<div class="list-view-rounded-corners">
		<table id = "tab_complaints" cellpadding='0' cellspacing='0' border='0' class='list view table-responsive'>
	<thead>
...
var fuzzyNum =function(x){
 ...
 $.fn.dataTableExt.oSort['numbercomplaint-asc']=function(x, y){
    return fuzzyNum(x)- fuzzyNum(y);
 }; 
  
 $.fn.dataTableExt.oSort['numbercomplaint-desc']=function(x, y){
    return fuzzyNum(y)- fuzzyNum(x);
 };

$('#tab_complaints').DataTable( {
		columnDefs : [
			 {
			"searchable": false,
			"orderable": true,
        ...
        initComplete: function () {
           this.api().columns().every( function () {
				...
				var complaint_status = SUGAR.language.get('app_list_strings', 'complaint_status_dom');
				this.api().columns([4]).every( function () {
					var column = this;
					var select = $('<select multiple   id = "status" class="dt-search-select"><option value="">(No filter applied)</option></select>')
						.prependTo( $(column.header()));
										
					$.each(complaint_status, function( index, value ) {
				...
				this.api().columns([6]).every( function () {
					...
					var select = $('<select class="dt-search-select"><option value="">(No filter applied)</option></select>')
						...
						var users = SUGAR.language.get('app_list_strings', 'assigned_lawyer_complaints_list');
