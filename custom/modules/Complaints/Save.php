...
if (isCloseAndCreateNewPressed()) $focus->status = 'Closed';
...
if(isset($_REQUEST['inbound_email_id']) && !empty($_REQUEST['inbound_email_id'])) {
	// fake this complaint like it's already saved.
	$focus->save();
...
// parent_id are the same value (in which complaint it should just use relate_id) by adding the != check
...
handleRedirect($return_id,'Complaints');
?>
