...
class GoogleSync
{
    ...
    public function getUserMeetings()
    {
        ...
        $query = "SELECT id FROM fp_events WHERE assigned_user_id = '" . $this->workingUser->id . "' AND date_start <= now() + interval 3 month";
    ...
    public function getMeetingByEventId($event_id)
    {
        ...
        // We do it this way so we also get deleted meetings
        $query = "SELECT id FROM FP_events WHERE gsync_id = '{$event_id}'";
        $result = $this->db->query($query);
    ...
    public function pushPullSkip($event_1, $event_2 = null)
    {
        ...
        if (!isset($event_2) || empty($event_2)) {
            switch (get_class($event_1)) {
                case "Google\Service\Calendar\Event":
            ...
            if ($obj_class1 == 'FP_events') {
                $event_local = $event_1;
            } elseif ($obj_class1 == 'Google\Service\Calendar\Event') {
            ...
            if ($obj_class2 == 'FP_events') {
                $event_local = $event_2;
            } elseif ($obj_class2 == 'Google\Service\Calendar\Event') {
     ...
     */
    public function pullEvent($event_remote, $event_local = null , $creatorEmail)
    {
        if (!isset($event_local) || empty($event_local)) {
            $event = $this->createSuitecrmMeetingEvent($event_remote);
            $this->sendEmailToCreator($event , $creatorEmail);
        } else {
    ...
    public function delEvent($event, $meeting_id)
    {
        ...
        if ($statusCode >= 200 && $statusCode <= 299) {
         // 2xx statusCode = success
            ...
            $sql = "UPDATE fp_events SET gsync_id = '' WHERE id = '" . $meeting_id . "'";
    ...
    public function updateSuitecrmMeetingEvent($event_local, $event_remote)
    {
        ...
        $event_local->date_start = date("m/d/Y h:i", $starttime);
        $event_local->date_end = date("m/d/Y h:i", $endtime);
        $event_local->duration_hours = $hours;
        $event_local->duration_minutes = $mins;
        $event_local->assigned_user_id = $this->workingUser->id;
        $event_local->multiple_assigned_users = '^'.$this->workingUser->id.'^';
        ...
        foreach ($meetings as $meeting) {
            ...
            switch ($dowhat) {
                ...
                case "pull":
                    ...
                    $this->pullEvent($gevent, $meeting , $creatorEmail);
                    break;
                case "skip":
...
// print_r($googleEvents);die();
        foreach ($googleEvents as $gevent) {
            $creatorEmail = $gevent->getCreator()->getEmail();
    ...
    public function sendEmailToCreator($event ,$creatorEmail){
        $FP_events_calendar=$event;
        global $db,$sugar_config;
        $email=$creatorEmail;
        $site_url = $sugar_config['site_url'];
                    $query=" SELECT *
                    FROM cases_fp_events_1_c
                    WHERE cases_fp_events_1_c.cases_fp_events_1fp_events_idb = '".$FP_events_calendar->id."'";
            $result=$db->query($query);
            if($result->num_rows==0){           
            $to =$email;
            $subject ="New Calendar Entry in Honey";
            $htmlContent = "New Calendar Meeting Event Sync with Honey with Name" . " " . "' <b>" . $FP_events_calendar->name . "</b> '" . " Please assigned this event to the case!";
            $htmlContent .= "<br><a href = '{$site_url}/index.php?module=FP_events&action=DetailView&record={$FP_events_calendar->id}'>Click to View Event</a>";
            $headers = "MIME-Version: 1.0" . "\r\n";
            $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
            $headers .= 'From: CRM <'.$email.'>' . "\r\n";        
