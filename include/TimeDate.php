...
class TimeDate
{
    ...
    protected function _convert($date, $fromFormat, $fromTZ, $toFormat, $toTZ, $expand = false)
    {
        ...
        try {
            ...
            if ($phpdate == false) {
                $GLOBALS['log']->fatal("convert: Conversion of $date from $fromFormat to $toFormat failed");
                return '';
            }
            ...
            $GLOBALS['log']->fatal("Conversion of $date from $fromFormat to $toFormat failed: {$e->getMessage()}");
            return '';
        }
    }
