...
class ListViewData {
	...
	function getListViewData($seed, $where, $offset=-1, $limit = -1, $filter_fields=array(),$params=array(),$id_field = 'id',$singleSelect=true, $id = null) {
        ...
        // print_r($ret_array);die();
        // print_r($params);die();
        // if($seed->module_dir=='Leads'){
        //     $main_query = $ret_array['select'] . $params['custom_select'] . $ret_array['from'] . $params['custom_from'] . $ret_array['inner_join']. $ret_array['where'] . $params['custom_where'] . " ORDER BY leads.leadrank DESC ";
        // }else{
        ...
        //}
		/* echo $main_query; */
        // print_r($main_query);die();
		/* $GLOBALS['log']->fatal('Tasks');
		...
		if(empty($_REQUEST['action']) || $_REQUEST['action'] != 'Popup') {
   		...
   		if($limit < -1) {
			...
			if($limit == -1) {
				if($this->seed->module_dir == 'Cases'){
					$limit = 50000;     // Showing all the entries for the cases module.
				}else{
					$limit = $this->getLimit();
				}
            }
