        ...
        for ($i = 0; $i < $line_count; ++$i) {
            ...
            if ($post_data[$key . 'deleted'][$i] == 1) {
                ...
                if ((!isset($condition->parenthesis) || !$condition->parenthesis) && isset($condition->value_type) && $condition->value_type == 'Period') {
                    $condition->value = base64_encode($_POST['aow_conditions_value'][$i]);
                }
                if (trim($condition->field) != '' || $condition->parenthesis) {
                    if (isset($_POST['aow_conditions_order'][$i])) {
                        $condition->condition_order = (int)$_POST['aow_conditions_order'][$i];
                    } else {
