...
* this program; if not, see www.gnu.org/licenses or write to the Free
...
class FP_eventsFormBase extends FormBase {
...
$lbl_required_symbol = $app_strings;
$lbl_date = $mod_strings;
$lbl_time = $mod_strings;
$ntc_date_format = $timedate->get_user_date_format();
$ntc_time_format = '('.$timedate->get_user_time_format().')';

$user_id = $current_user->id;
// $default_status = $app_list_strings;
$default_parent_type= $app_list_strings;
...
<input type="hidden" name="${prefix}duration_hours" value="0">
...
<input name='${prefix}date_start' id='jscal_field' onblur="parseDate(this, '$cal_dateformat');" type="text" maxlength="10" value="${default_date_start}"> <!--not_in_theme!--><img src="{$jscalendarImage}" alt="{$app_strings}" id="jscal_trigger" align="absmiddle"><br>
...
$javascript->setSugarBean(new FP_events());
...
function getForm($prefix, $mod='FP_events'){
if(!ACLController::checkAccess('FP_events', 'edit', true)){
return '';
}
...
$lbl_save_button_title = $app_strings;
$lbl_save_button_key = $app_strings;
$lbl_save_button_label = $app_strings;


$the_form = get_left_form_header($mod_strings);
$the_form .= <<<EOQ


<form name="${prefix}FP_eventsSave" onSubmit="return check_form('${prefix}FP_eventsSave')" method="POST" action="index.php">
<input type="hidden" name="${prefix}module" value="FP_events">

<input type="hidden" name="${prefix}action" value="Save">

EOQ;
$the_form	.= $this->getFormBody($prefix, 'FP_events',"{$prefix}FP_eventsSave" );
$the_form .= <<<EOQ
...
function handleSave($prefix,$redirect=true, $useRequired=false) {
	...
	//add assigned user and current user if this is the first time bean is saved
  	if(empty($focus->id) && !empty($_REQUEST['return_module']) && $_REQUEST['return_module'] =='FP_events' && !empty($_REQUEST['return_action']) && $_REQUEST['return_action'] =='DetailView'){
	...
	if (isset($_REQUEST['return_module']) && $_REQUEST['return_module'] == 'Home'){
		...
		handleRedirect($return_id, 'FP_events');
	} else {
		return $focus;
	}

} // end handleSave(); // end handleSave();
