...
class EmailsController extends SugarController
{
    ...
    public function action_ImportAndShowDetailView2($messageNo,$uid)
    {
        global $db;
           // $inboundEmail = new InboundEmail();
            $InboundEmailBean     = BeanFactory::getBean('InboundEmail');
            $query="inbound_email.status='Active'  AND inbound_email.email_user='devmbm2@gmail.com'";
            $InboundEmailBeanList = $InboundEmailBean->get_full_list("", $query);
// pre($InboundEmailBeanList);
            $InboundEmailBean->retrieve($db->quote($InboundEmailBeanList[0]->id), true, true);
            $InboundEmailBean->connectMailserver();
            $importedEmailId = $InboundEmailBean->returnImportedEmail($messageNo, $uid);
