...
EOJS;
   ...
   function getMassGenerateDocumentForm() {
      ...
      $html .= "<input type='hidden' name='action' id='MGD_action' value='htgeneratedocument'>\n";  // acción de Sugar, ver modules\DHA_PlantillasDocumentos\controller.php
...
EOJS;
		...
		if($_REQUEST['module'] == 'Cases'){
			$html .= "<label style='vertical-align: top; line-height: 19px; height: 19px; margin: 2px 0; display: block; float:left;'><input type='checkbox' style='width: 19px; height: 19px; vertical-align: bottom;' name='AttachGeneratedDocumentToRecord' id='AttachGeneratedDocumentToRecord' value='1' checked='checked'>&nbsp; {$GLOBALS['app_strings']['LBL_ADJUNTAR_DOCUMENTO_GENERADO_A_CASE']}</label>"; 
		}         
      ...
      //if(!preg_match("/$match_version/", $sugar_version)) {
		...
		/*  HT Customizations Start*/
        $category_list = convertArrayToITL($app_list_strings['dha_plantillasdocumentos_category_dom']);
		foreach($app_list_strings['ht_subcategory_relation_dom'] AS $value => $list_name ){
			$subcategory_list[$value] = convertArrayToITL($app_list_strings[$list_name],false);
		}
		$subcategory_list = json_encode($subcategory_list);
		$pleading_type_list = convertArrayToITL($app_list_strings['pleading_type_list']);
		$js_file_path = getJSPath('modules/DHA_PlantillasDocumentos/MassGenerateDocument.js');       
		$js_loading_file = getJSPath('custom/include/javascript/loadingoverlay.min.js');       
		
		$javascript = <<<EOJS
			  <script type="text/javascript" src="{$js_file_path}"></script>
			  <script type="text/javascript" src="{$js_loading_file}"></script>
		...
		<script>
			...
			var pleading_type_list	 = {$pleading_type_list};
		</script>
EOJS;
/*  HT Customizations End*/
      $html .= $javascript;
      if ($accion == 'DetailView')
		...
		<div class="modal scale" id="myModal" role="dialog">
			<div class="modal-dialog" style="max-height: 1350px; width: 90%;">
				<div class="modal-content" style="height: 550px;max-width: 1350px;width: 90%;">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal">&times;</button>
						<h4 class="modal-title">Generate Document</h4>
					</div>
					<div class="modal-body" style= "height: 73%;">
						<form action="index.php" target="_blank" method="POST" name="document_generation_form" id="document_generation_form"  enctype="multipart/form-data">
							
							<div class="row" style="margin-right: 0px; margin-left: inherit;">
								<div class="col-xs-4 col4 select_area">
									<div class="title">Category</div>
									<select id="document_categories">
									</select>
									
								</div>
								<div class="col-xs-4 col-4 select_area" id="document_subctg_section" style= "display: none;">
									<div class="title">Subcategory List</div>
									  <select id="document_subctg">
									</select>
								</div>
								<div class="col-xs-4 col-4 select_area" id="pleading_types_section" style= "display: none;">
									<div class="title">Pleading Types</div>
									  <select id="pleading_types">
									</select>
								</div>
								<div class="col-xs-4 col-4 select_area" id="document_list_section" style= "display: none;">
									<div class="title">Document List</div>
									  <select id="document_list" name="plantilladocumento_id">
									</select>
								</div>
								<div class="col-xs-4 col-4 select_area" id="letterhead_list_section" style= "display: none;">
									<div class="title">Letterhead List</div>
							...
							<div class="row" style="margin-right: 0px; margin-left: inherit; display: none; overflow: hidden;">		
							';
								  $html .= "<input type='hidden' name='moduloplantilladocumento' id='MGD_moduloplantilladocumento' value='{$modulename}'>\n"; // realmente este sería innecesario, se puede obtener a través de plantilladocumento_id
								  $html .= "<input type='hidden' name='module' id='MGD_module' value='DHA_PlantillasDocumentos'>\n";
								  $html .= "<input type='hidden' name='action' id='MGD_action' value='htgetSelectionPanelJSON'>\n";  // acción de Sugar, ver modules\DHA_PlantillasDocumentos\controller.php
								  $html .= "<input type='hidden' name='mode' id='MGD_mode' value=''>\n";     // este se rellena por el javascript (entire o selected)
								  $html .= "<input type='hidden' name='enPDF' id='MGD_enPDF' value='false'>\n";
								  $html .= "<input type='hidden' name='from_view'  value=''>\n";
								  $html .= "<input type='hidden' name='related_selected'  value=''>\n";
								  $html .= "<input type='hidden' name='AttachGeneratedDocumentToRecord'  value=''>\n";
								 if ($accion == 'ListView')       
									 $html .= "<input type='hidden' name='uid' id='MGD_uid' value=''>\n";   // este se rellena por el javascript (lista de las ids seleccionadas, a no ser que el mode sea igual a entire)
								  if ($accion == 'DetailView') 
									 $html .= "<input type='hidden' name='uid' id='MGD_uid' value='{$this->sugarbean->id}'>\n";  // si estamos en el DetailView, no se rellena por javascript, ya sabemos su valor
					...
					<div class="modal-footer">
						<label><input type="checkbox"  name="AttachGeneratedDocumentToRecord" id="AttachGeneratedDocumentToRecord_detailview" value="1" checked="checked">&nbsp; Attach Document with Record &nbsp;</label>
						<button type="button" class="btn btn-default"  onclick="document_generation_form_submission();">Generate Document</button>
						<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
					</div>
					</form>
				</div>
			</div>
		</div><style>.select2-results__options{height: 500px !important;max-height: 500px !important;}</style>';
		/*  HT Customizations End */      
      if($row_count > 0) {
         return $html;
      } else {     
         ...
         $html = '';
         $html .= "<div id='massgeneratedocument_form' class='no-template' {$no_visible}>";
         $html .= "<br>";   
         $html .= "<table width='100%' cellpadding='0' cellspacing='0' border='0' class='formHeader h3Row'><tr><td nowrap><h3><span>" . $app_strings['LBL_GENERAR_DOCUMENTO']."</h3></td></tr></table>";       
   ...
   function handleMassGenerateDocument(){
      ...
      if(isset($_POST['MassGenerateDocument_ids']) && is_array($_POST['MassGenerateDocument_ids'])){
         ...
         if (isset($_REQUEST['AttachToEmailGeneratedDocument']) && $_REQUEST['AttachToEmailGeneratedDocument']) {
            ...
            GenerateDocumentAndAttachToRecord ($_REQUEST['moduloplantilladocumento'], $_REQUEST['plantilladocumento_id'], $_POST['MassGenerateDocument_ids'], $enPDF);
         }
   ...
   ///////////////////////////////////////////////////////////////////////////////////////////////////      
   function HThandleMassGenerateDocument(){
      ...
      if(isset($_POST['MassGenerateDocument_ids']) && is_array($_POST['MassGenerateDocument_ids'])){
         require_once("modules/DHA_PlantillasDocumentos/Generate_Document.php");
			...
			return array($_REQUEST['moduloplantilladocumento'], $_REQUEST['plantilladocumento_id'], $_POST['MassGenerateDocument_ids'], $enPDF);       
      }
   ...
   function handleMassGenerateDocument_Sugar7($api, $args){
      ...
      if(isset($args['attach_to_case']) && !in_array(strval($args['attach_to_case']), $boolean_false_values)) 
         $attach_to_case = true;
      ...
      if(is_array($uid) && count($uid) > 0){
         ...
         if ($attach_to_email) {
		  ...
		  else if ($attach_to_case) {
            $note_id_case = GenerateDocumentAndAttachToRecord ($modulename, $template_id, $uid, $inPDF);
            return array('note_id_case' => $note_id_case);
         }
