...
require("modules/DHA_PlantillasDocumentos/Generate_Presentation.php");
   ...
   function GenerarInforme () {
      ...
      $this->CalculatedFieldsClass->BeforeMergeBlock();  // Evento
		...
		if ($this->is_PowerPoint) { // MSOffice Handling
			// $count_slides = $this->TBS->PlugIn(OPENTBS_COUNT_SLIDES); 
			// if ($count_slides > 0) { // MSOffice Handling
				// for($slide=1;$slide < $count_slides;$slide++) {
					// $this->TBS->PlugIn(OPENTBS_SELECT_SLIDE, $slide);
				
					

					// //print"<pre>";print_r($this->datos );die;
			
					// $this->TBS->MergeBlock('a',$this->datos);
				// }
			// }
			// //$this->TBS->Show(OPENTBS_STRING);
			// $this->TBS->Plugin(OPENTBS_ADDFILE, 'ppt/slides/slide3.xml', $string);
			// //$this->TBS->Plugin(OPENTBS_DEBUG_XML_SHOW);
			// // die;
			$presentGraph = new PHPPresentationGraph($_REQUEST['moduloplantilladocumento'], $_POST['MassGenerateDocument_ids']);      
			$presentGraph->generatePresentation($this->modulo,$this->ids);  
		}else{
			$this->TBS->MergeBlock('a', $this->datos);
		}
		//HT_Custom_Block_Start
		// $this->TBS->Plugin(OPENTBS_SELECT_HEADER); // load OpenTBS plugin
		// $this->TBS->MergeBlock('a', $this->datos);
		//HT_Custom_Block_End
...
EOJS;
   ...
   ///////////////////////////////////////////////////////////////////////////
   function ObtenerDatos ($pre_selected = array()) {
      ...
      foreach($this->ids as $id){
       ...
       //  print"<pre>";print_r($linked_fields);die;
				...
				foreach($related_beans as $key => $related_bean){
                  ...
                  foreach($related_bean->field_name_map as $campo){
					...
					if($_REQUEST['from_view'] == 'customselection'){
						if(in_array($related_bean->id,$pre_selected)){
