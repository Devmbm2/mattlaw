...
class DHA_PlantillasDocumentos extends File {
   ...
   ///////////////////////////////////////////////////////////////////////////////////////////////////   
   function get_file_name($id, $file_ext, $update_file = true) {
      global $sugar_config;
	if(!empty($this->doc_url)  && $update_file){
		$gFileId = $this->getIdFromUrl($this->doc_url);
		if(!empty($gFileId[0])){
			$result = array();
			require_once('include/externalAPI/ExternalAPIFactory.php');
			try {
				$api = ExternalAPIFactory::loadAPI($this->doc_type,true);
				$this->file_mime_type = empty($this->file_mime_type) || $this->file_mime_type == 'application/octet-stream' ? 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' : $this->file_mime_type;
				if(empty($file_ext) && $this->file_mime_type == 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' ){
					$file_ext = $this->file_ext = 'docx';
				}
				if (isset($api) && $api !== false) {
					$response = $api->getFileContents($gFileId[0], $this->file_mime_type);	
				} else {
					$result['success'] = false;
					// FIXME: Translate
					$GLOBALS['log']->fatal("Could not load the requested API (" . $this->doc_type . ")");
   ...
   function save($check_notify = false) {
      global $mod_strings;
	  if (!empty($this->file_url)) $this->doc_url = $this->file_url;
      // Bloqueo inicial, por el tipo de archivo
      if (isset($this->file_ext) && !empty($this->file_ext)) {
         if (!in_array($this->file_ext, array('docx', 'odt', 'docm', 'xlsx', 'ods', 'xlsm', 'pptx'))) {
   ...
   function fill_in_additional_detail_fields() {
      ...
      $plantilla = getcwd() . "/". $this->get_file_name($this->id, $this->file_ext, false);
      if (is_file($plantilla)) {
      ...
      if ($ExistePlantilla) {  
         ...
         if($this->ACLAccess('DetailView')){
            ...
            // $this->file_url = "<a href='index.php?entryPoint=download_dha_document_template&type=DHA_PlantillasDocumentos&id=".$this->id."' target='_blank'>".SugarThemeRegistry::current()->getImage($img_name, 'alt="'.$mod_strings['LBL_LIST_VIEW_DOCUMENT'].'"  border="0"')."</a>";
         } 
      }
      ...
      if ($ExistePlantilla) {         
         $this->doc_local_location = $this->get_file_name($this->id, $this->file_ext, false);      
      } else {
   ...
   function create_new_list_query($order_by, $where,$filter=array(),$params=array(), $show_deleted = 0,$join_type='', $return_array = false,$parentbean=null, $singleSelect = false, $ifListForExport = false) {   
      ...
      if($_REQUEST['module'] == 'DHA_PlantillasDocumentos' && $_REQUEST['action'] == 'DHA_PlantillasDocumentos'){
		  $filtro_modulos = '';
