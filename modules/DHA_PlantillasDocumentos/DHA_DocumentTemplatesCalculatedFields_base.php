...
class DHA_DocumentTemplatesCalculatedFields {
   ...
   private function SetCalcFieldsDefs_base() {
      ...
      if (isset($this->bean->field_name_map['currency_id'])) { 
      ...
      if ($this->inMainModule()) { 
		 ...
		 $this->CalcFieldsDefs_base['current_year'] = array (
            'name' => 'current_year',
            'vname' => 'LBL_CURRENT_YEAR',
            'type' => 'calculated_date',
            'help' => 'Fecha actual',         
         );
		 ...
		 $this->CalcFieldsDefs_base['current_day'.$this->suffix_inWords] = array (
            'name' => 'current_day'.$this->suffix_inWords,
            'vname' => 'LBL_CURRENT_DAY',
            'type' => 'calculated_date',
            'help' => '',         
         );
		 $this->CalcFieldsDefs_base['current_month'.$this->suffix_inWords] = array (
            'name' => 'current_month'.$this->suffix_inWords,
            'vname' => 'LBL_CURRENT_MONTH',
            'type' => 'calculated_date',
            'help' => '',         
         );
      }
   ...
   function UpdateBeanFieldNameMap() {
      ...
      foreach ($this->CalcFieldsDefs as $key => $field) {
         ...
         $this->bean->field_name_map[$field['name']] = $field;
      }      
   }  
   ...
   function CalcFields() {
      ...
      if ($this->inMainModule()) {
         ...
         $this->SetCalcValue('current_year', $this->Generate_Document_Instance->FormatDateByYear(time()));
         $this->SetCalcValue('current_date'.$this->suffix_inWords, $this->Generate_Document_Instance->FormatearFechaLarga(time()));  
         $this->SetCalcValue('current_day'.$this->suffix_inWords, $this->Generate_Document_Instance->FormatearDayWord(time()));  
         $this->SetCalcValue('current_month'.$this->suffix_inWords, $this->Generate_Document_Instance->FormatearMonthWord(time()));  
