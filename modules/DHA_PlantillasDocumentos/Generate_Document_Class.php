...
if(!defined('sugarEntry') || !sugarEntry) die('Not A Valid Entry Point');
	// require_once('custom/include/DocxMerge/vendor/autoload.php');
	// use DocxMerge\DocxMerge;
	require_once 'custom/include/DocxMerge/phpdocx/vendor/autoload.php';
class Generate_Document {
   ...
   public $relaciones_relaciones_related = array();
   ...
   function __destruct() {
	...
	function parsePrettyTags(){
		...
		require_once('modules/DHA_PlantillasDocumentos/Parse_Pretty_Tags.php');
		if(!empty($_REQUEST['template_html'])){
		   $this->TBS->Source = $_REQUEST['template_html'];
		   $this->is_Document = true;
		   $this->template_file_ext = 'docx';
		}
		$pt = new clsParsePrettyTags($this->TBS->Source);
		$this->TBS->Source = $this->template_source = $pt->ParsePrettyTags();
   ...
   function SetVariablesFormatos () { 
	 ...
	 if (isset($this->lang_format_config[$this->idioma]['year_format']))
         $this->idiomaYearFormat = $this->lang_format_config[$this->idioma]['year_format'];
      else 
         $this->idiomaYearFormat = $this->lang_format_config[$this->idiomaDefecto]['year_format'];           
   ...
   function AsignarVariablesTipoPlantilla ($file_ext) {
      $this->isOfficeOpenXML = ($file_ext == 'docx' || $file_ext == 'docm' || $file_ext == 'xlsx' || $file_ext == 'xlsm'|| $file_ext == 'pptx');
      $this->isOpenDocument = ($file_ext == 'odt' || $file_ext == 'ods');  

      $this->is_Document = ($file_ext == 'docx' || $file_ext == 'docm' || $file_ext == 'odt' || $file_ext == 'pptx');
      $this->is_PowerPoint = ($file_ext == 'pptx');
   ...
   function CargarPlantilla () {
	...
	if(empty($_REQUEST['template_html'])){
	  ...
	  // if(!empty($_REQUEST['letterhead_id'])){
		 $header_id = $_REQUEST['letterhead_id'];
		// $header_id = $current_user->default_letterhead;
		if(($this->template_file_ext == 'docx' || $this->template_file_ext == 'doc') && $this->bean_plantilla->use_letterhead != 'no' && $this->bean_plantilla->litigation_c != 'Pleadings' && !empty($header_id) && $_REQUEST['action'] == 'htgeneratedocument'){
			if($this->bean_plantilla->use_letterhead == 'yes'){
         $header_bean = SugarModule::get('ht_doc_temp_letter_head')->loadBean();
         }
         else{
         $header_bean = SugarModule::get('ht_doc_temp_multi_letterhead')->loadBean();
            }
			$header_bean->retrieve($header_id);
         // echo $header_id;
         // die();
			if(isset($header_bean->id)){
				if($this->bean_plantilla->use_letterhead == 'multiple' && $header_bean->ht_doc_tem2268terhead_ida){
					$header_bean_multi = SugarModule::get('ht_doc_temp_multi_letterhead')->loadBean();
					$header_bean_multi->retrieve($header_bean->ht_doc_tem2268terhead_ida);
					$header_download_location = $header_bean_multi->get_file_name($header_id, $header_bean_multi->file_ext);
					$temp_dowload_loc = 'document_templates/MERG-'.$header_bean_multi->id.'.'.$header_bean_multi->file_ext;
				}else{
					$header_download_location = $header_bean->get_file_name($header_id, $header_bean->file_ext);
					// die($header_download_location);
					$temp_dowload_loc = 'document_templates/MERG-'.$header_bean->id.'.'.$header_bean->file_ext;
				}
				if(file_exists($header_download_location) && file_exists($this->template_download_location)){
					$merge = new Phpdocx\BatchProcessing\MultiMerge();
					$merge->mergeDocx($header_download_location, array($this->template_download_location), $temp_dowload_loc, array('mergeType' => 1));
					sugar_chmod($temp_dowload_loc, 0777);
					$this->template_download_location =  $temp_dowload_loc;
					$this->temp_files[] =  $temp_dowload_loc;
				}
			}
		}
	  // }
     ...
     $this->parsePrettyTags();
      ...
      if (preg_match_all("/a\.(\w*){$this->separador_campo_related}/i", $this->template_source, $coincidencias)) {
         ...
         foreach ($relaciones_related as $relacion) {
			...
			//Check Relation for 3rd Level
			if (preg_match_all("/a\.{$relacion}{$this->separador_campo_related}(\w*){$this->separador_campo_related}/i", $this->template_source, $coincidencias)) {
				 $relaciones_relaciones_related = $coincidencias[1];
				 $relaciones_relaciones_related = array_unique ($relaciones_relaciones_related);
				 
				 foreach ($relaciones_relaciones_related as $relacion_relacion) {
					$this->relaciones_relaciones_related[$relacion][] = $relacion_relacion;
				 }
				 $this->relaciones_relaciones_related[$relacion] = array_unique ($this->relaciones_relaciones_related[$relacion]);
			}
         }
      }
   ...
   ///////////////////////////////////////////////////////////////////////////
   function GenerarInforme ($preview = false) {
      global $mod_strings, $sugar_config;
		// Merge data
		$this->CalculatedFieldsClass->BeforeMergeBlock();  // Evento
	 	// if(sizeof($this->datos) > 1){
			// foreach($this->datos AS $datos){
				// $this->GenerarInformeMultipleFiles(array('1' => $datos));
			// }
			// die;
		// }
		if ($this->is_PowerPoint) { // MSOffice Handling
			$document_bean = BeanFactory::getBean('DHA_PlantillasDocumentos', $_REQUEST['plantilladocumento_id']);
			require_once("modules/DHA_PlantillasDocumentos/Generate_Presentation.php");
			$presentGraph = new PHPPresentationGraph($_REQUEST['moduloplantilladocumento'], $_POST['MassGenerateDocument_ids'], $this->Download);   
     		if($document_bean->document_name == 'timeline bar chart2.pptx'){
				$ppt_result = $presentGraph->generatePresentationBarChart();
					
			}else{
				$ppt_result = $presentGraph->generatePresentationPieChart();
			}
			
			$this->template_filename = $ppt_result['file_name'];
			$this->Download_filename = $ppt_result['file_address'];
      ...
      if ($preview){
		  ob_clean();
		  echo html_entity_decode($this->TBS->Source);die;
	}
      if ($this->enPDF){
         ...
         elseif (!is_windows() && isset($sugar_config['DHA_OpenOffice_cde']) && $sugar_config['DHA_OpenOffice_cde'] && file_exists($sugar_config['DHA_OpenOffice_cde']) && is_readable($sugar_config['DHA_OpenOffice_cde'])){            
            ...
            $file_name_pdf = $this->GetTempFileName($info['filename'], 'pdf', '');
			...
			$fichero_destino = $this->includeTrailingCharacter ($sugar_config['tmp_dir'], '/') . $file_name_sandbox;
			copy($file_name, $fichero_destino);    
			$pdf_command = $sugar_config['DHA_OpenOffice_cde'].' --headless --nologo --nofirststartwizard --convert-to pdf '.$this->includeTrailingCharacter ($sugar_config['site_dir'], '/').$fichero_destino.' --outdir '.$info['dirname'];
            ...
            if ($this->Download) {
               $this->DescargarInforme ($file_name_pdf);
				...
				// copy($this->includeTrailingCharacter ($info['dirname'], '/').$info['filename'].'.pdf', $file_name_pdf);    
               $this->Download_filename = $file_name_pdf;
			   // echo $info['filename'].' ';
			   // echo $info['dirname'].$file_name_sandbox.' ';
			   // die($pdf_command);
               $this->Download_mimetype = $this->GetMimeTypeFromFileName($this->Download_filename);
            }            
	...
	function GenerarInformeMultipleFiles ($datos) {
      global $mod_strings, $sugar_config;
		// Merge data
      ...
      if ($this->enPDF){
		 ...
		 if ($this->Download) {
			 $file_name = $this->template_filename;
			$this->TBS->Show(OPENTBS_FILE, $file_name);
         }
         else {
            $file_name = $this->GetTempFileName('', $this->template_file_ext, '');
   ...
   function htImageField(&$related_bean, &$nombre_campo){
		$f_val = "upload/".$related_bean->id.'_'.str_replace(" ","_",$related_bean->$nombre_campo);
		if(copy("upload/{$related_bean->id}_{$nombre_campo}", $f_val)){
			$this->temp_files[] = $f_val;
			return $f_val;
		}
		return '';
   } 
...
EOQ;
      ...
      $cuenta = 0;
      foreach($bean->field_name_map as $campo){
         ...
         if ($this->TipoDatoValido($tipo, $nombre_campo)){    
            ...
            if (isset($campo['calculated']) && $campo['calculated'])
...
EOJS;
   ...
   function EtiquetaLink ($LinkFieldName, $LinkFieldProperties, $LinkObject) { 
      ...
      if (get_class($LinkObject) == 'Link') {
         ...
         if (!$EsRelacionDeSubPanel) {
            foreach ($this->bean_datos->field_name_map as $field_name => $field_properties) {   
   ...
   function ObtenerDatos () {
      ...
      foreach($this->ids as $id){
        ...
        foreach($this->bean_datos->field_name_map as $campo){
            $nombre_campo = $campo['name'];
            $tipo = '';
            if (isset($campo['type']))
               $tipo = $campo['type'];
        ...
        //HT_Custom_Block_Start
        if($this->bean_datos->module_name == 'Cases' ){
			require_once 'custom/include/OPENTBSCasesConfig.php';
			if(isset($openTBSConfigCases[$this->bean_datos->source_c])){
				$this->datos[$cuenta] = array_merge($this->datos[$cuenta],$openTBSConfigCases[$this->bean_datos->source_c]);
			}
			
		}
		$this->datos[$cuenta]['current_user@@initials_c'] = $current_user->initials_c;
		//HT_Custom_Block_End
         ...
         foreach ($linked_related_fields as $key => $related_field_name) {
            ...
            if (empty($related_beans)) {
               ...
               foreach($modulo_bean->field_name_map as $campo){
                  $nombre_campo = $campo['name'];
				...
				// 3rd level blank 
				if(isset($this->relaciones_relaciones_related[$related_field_name])){
					  foreach($this->relaciones_relaciones_related[$related_field_name] AS $relation){
						  //Load the relationship
							$temp_bean = BeanFactory::getBean($related_bean->module_dir, $related_bean->id);
							if($temp_bean->load_relationship($relation)){
								$thirdModuleName = $temp_bean->$relation->getRelatedModuleName();
								$thirdRelBean = BeanFactory::newBean($thirdModuleName);
								foreach($thirdRelBean->field_name_map as $campo){
									 $nombre_campo = $campo['name'];
									 $nombre_campo_TBS = $related_field_name . $this->separador_campo_related .$relation. $this->separador_campo_related . $campo['name'];
									 $tipo = '';
									 if (isset($campo['type']))
										$tipo = $campo['type'];
               ...
               foreach($related_beans as $key => $related_bean){
                  ...
                  foreach($related_bean->field_name_map as $campo){
                     $nombre_campo = $campo['name'];
                     $nombre_campo_TBS = $related_field_name . $this->separador_campo_related . $campo['name'];
                     $tipo = '';
                     if (isset($campo['type']))
                     ...
                     if ($this->TipoDatoValido($tipo, $nombre_campo)){
                        ...
                        if (isset($related_bean->$nombre_campo)){
							if($tipo == 'image' && file_exists("upload/{$related_bean->id}_{$nombre_campo}")){
								$this->datos[$cuenta][$nombre_campo_TBS] = $this->htImageField($related_bean, $nombre_campo);
							}else{
				  ...
				  if(isset($this->relaciones_relaciones_related[$related_field_name])){
					  foreach($this->relaciones_relaciones_related[$related_field_name] AS $relation){
						  //Load the relationship
							$temp_bean = BeanFactory::getBean($related_bean->module_dir, $related_bean->id);
							if($temp_bean->load_relationship($relation)){
								$thirdRelBeans = $temp_bean->$relation->getBeans();
								foreach($thirdRelBeans AS $thirdRelBean){
									foreach($thirdRelBean->field_name_map as $campo){
										 $nombre_campo = $campo['name'];
										 $nombre_campo_TBS = $related_field_name . $this->separador_campo_related .$relation. $this->separador_campo_related . $campo['name'];
										 $tipo = '';
										 ...
										 if ($this->TipoDatoValido($tipo, $nombre_campo)){
											$this->datos[$cuenta][$nombre_campo_TBS] = '';
											if (isset($thirdRelBean->$nombre_campo)){
												if($tipo == 'image' && file_exists("upload/{$thirdRelBean->id}_{$nombre_campo}")){
													$this->datos[$cuenta][$nombre_campo_TBS] = $this->htImageField($thirdRelBean, $nombre_campo);
												}else{
												   $this->datos[$cuenta][$nombre_campo_TBS] = $this->FormateaCampo($thirdRelBean->$nombre_campo, $campo, false);
												}
											}
										 }
									  }
        ...
        foreach($this->relaciones AS $relationships){
			
			if(strpos($relationships, '.') !== false){
				$related_bean = $this->bean_datos;
				
				$relate_array = explode('.', $relationships);
				 // print"<pre>";print_r($relate_array );die;		 

				foreach($relate_array AS $relation){
					$relation_conditions = explode(':', $relation);
					$relation_link = $relation_conditions[0];
					if($related_bean->load_relationship($relation_link)){
						$params = array();
						if(isset($relation_conditions[1]) && isset($relation_conditions[2])){
							$params = array(
								'where' => array(
									'lhs_field' => $relation_conditions[1],
									'operator' => '=',
									'rhs_value' => $relation_conditions[2]
								)
							);
						}
						$related_bean = array_pop($related_bean->$relation_link->getBeans($params));
					}
				}
				if($related_bean->id !== $this->bean_datos->id){
					$this->mapFieldsToValues($relationships, $related_bean, $cuenta, true);
				}
			}
		}  
            ...
            if (empty($related_beans)) {
               ...
               $this->datos[$cuenta][$related_field_name] = null;
				...
				foreach($related_beans as $key => $related_bean){
					...
					$this->mapFieldsToValues($related_field_name, $related_bean, $cuenta);
					
               }
            }
            unset($related_beans);
         }
		// Add support for Closing statement calculations
		// $medical_providers_list = array('a0c3da68-1708-0412-16dc-59784126010d', 'c830fb13-7766-b264-9dc4-59f6ef827cb4', 'b2a5073f-7e24-154d-2745-59797cdcc0d5');
        // if($_REQUEST['action'] != 'htgetSelectionPanelJSON'){
			require_once('modules/DHA_PlantillasDocumentos/custom_queries.php');
			if(isset($custom_queries_list[$this->modulo])){
				$res_custom_panel = array_intersect($custom_queries_list[$this->modulo],$this->relaciones);
				foreach($res_custom_panel AS $ht_related_field_name){
					$this->datos[$cuenta][$ht_related_field_name] = array();
					$sql = $custom_queries[$ht_related_field_name];
					
					$rs = $db->query($sql);
					if($_REQUEST['from_view'] == 'customselection'){
						while($row = $db->fetchByAssoc($rs)){
							if(in_array($row['id'], $this->RelatedSelected)){
								$this->datos[$cuenta][$ht_related_field_name][] = $row;
							}
						}
					}else{
						while($row = $db->fetchByAssoc($rs)){
   ...
   function mapFieldsToValues(&$related_field_name, &$related_bean, &$cuenta, $select_all = false){
		...
		if($_REQUEST['from_view'] == 'customselection'){
			if(in_array($related_bean->id,$this->RelatedSelected) || $select_all){
   ...
   function CrearPlantillaBasica () { 
      ...
      foreach ($Campos as $Campo) {
         ...
         if (strpos($Campo, $this->separador_campo_related) === false) {
            $etiqueta = $Campo;
            if (isset($this->bean_datos->field_name_map[$Campo]['vname'])) {
               $etiqueta = $this->translate ($this->bean_datos->field_name_map[$Campo]['vname'], $this->modulo);
               $SpreadSheetFieldType = $this->SpreadSheetFieldType($this->bean_datos->field_name_map[$Campo]);
            }
            ...
            $etiqueta = $nombre_campo;
            if (isset($modulo_bean->field_name_map[$nombre_campo]['vname'])) {
               $etiqueta = $this->translate ($modulo_bean->field_name_map[$nombre_campo]['vname'], $modulo);            
               $SpreadSheetFieldType = $this->SpreadSheetFieldType($modulo_bean->field_name_map[$nombre_campo]);
            }
            unset($modulo_bean);
            
            if (isset($this->bean_datos->field_name_map[$nombre_campo_relacion]['vname'])) {
               $etiqueta = $this->translate ($this->bean_datos->field_name_map[$nombre_campo_relacion]['vname'], $this->modulo) . ' - ' .  $etiqueta;
               if (!$SpreadSheetFieldType)
                  $SpreadSheetFieldType = $this->SpreadSheetFieldType($this->bean_datos->field_name_map[$nombre_campo_relacion]);
            }
            else {
      ...
      if ($this->is_Document){
         ...
         foreach ($Relaciones as $key => $Relacion) {
            ...
            foreach ($CamposDeRelacion[$Relacion] as $Campo) {
               $etiqueta = $Campo;
               if (isset($modulo_bean->field_name_map[$Campo]['vname'])) 
                  $etiqueta = $this->translate ($modulo_bean->field_name_map[$Campo]['vname'], $modulo);
   ...
   function FormatDateByYear($fecha) {
      // El parámetro $fecha será un timestamp
      
      $fecha = date($this->idiomaYearFormat, $fecha);
      return $fecha;
   }
   ...
   function FormatearDayWord($fecha) {
      $dia_semana = date('dS', $fecha);
      return $dia_semana;
   }
   function FormatearMonthWord($fecha) {
     
      $dia_semana = date('F', $fecha);
