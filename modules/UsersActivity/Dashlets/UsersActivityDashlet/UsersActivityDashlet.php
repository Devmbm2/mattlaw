...
class UsersActivityDashlet extends DashletGenericChart { 
    public $pcua_actions = array();
    ...
    public function __construct($id, array $options = null)
    {
        ...
        require('modules/UsersActivity/Dashlets/UsersActivityDashlet/UsersActivityDashlet.data.php');
        $this->_searchFields = $dashletData['UsersActivityDashlet']['searchFields'];
    ...
    public function displayOptions()
    {
        ...
        if (!empty($this->pcua_actions) && sizeof($this->pcua_actions) > 0)
            foreach ($this->pcua_actions as $key)
                $selected_datax[] = $key;
        else
            $selected_datax = array_keys($app_list_strings['usersactivity_actions']);
        
        $this->_searchFields['pcua_actions']['options'] = array_filter($app_list_strings['usersactivity_actions']);
        $this->_searchFields['pcua_actions']['input_name0'] = $selected_datax;
    ...
    public function display() 
    {
        ...
        $chartDef = array(	
            ...
            'groupBy' => array( 'action_name', 'name' ),
            'base_url'=> 
                    array( 	
            ...
            'url_params' => array( 'action_name', 'name', 'date_entered'),				
        );
    ...
    {
        $query = "SELECT action_name,name,count(*) as total FROM usersactivity ";
	...
	if ( count($this->pcua_ids) > 0 )
        ...
        if ( count($this->pcua_actions) > 0 )
            $query .= "AND usersactivity.action_name IN ('".implode("','",$this->pcua_actions)."') ";
	else
            $query .= "AND usersactivity.action_name IN ('".implode("','",array_keys($GLOBALS['app_list_strings']['usersactivity_actions']))."') ";
        ...
        $query .= " GROUP BY name,action_name ORDER BY action_name,name";
