...
class LeadsViewDetail extends ViewDetail {
 	...
 	function display(){
		...
		$this->ss->assign("DISABLE_CONVERT_ACTION", $disableConvert);
         	...
         	//hide field
		if($this->bean->source_c !== "Referral_from_Attorney"){
        	   unset($this->dv->defs['panels']['LBL_CONTACT_INFORMATION'][8][0]);
        	}
		if($this->bean->source_c !== "Referral_from_NonAttorney"){
        	   unset($this->dv->defs['panels']['LBL_CONTACT_INFORMATION'][8][1]);
        	}
		if($this->bean->status !== "Dead"){
        	   unset($this->dv->defs['panels']['LBL_CONTACT_INFORMATION'][11][1]);
        	}
