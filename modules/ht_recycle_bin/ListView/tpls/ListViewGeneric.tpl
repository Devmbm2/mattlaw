...
{include file='modules/ht_recycle_bin/ListView/tpls/ListViewColumnsFilterDialog.tpl'}
<script type='text/javascript' src='{sugar_getjspath file="include/javascript/popup_helper.js"}'></script>
<script type='text/javascript' src='{sugar_getjspath file="modules/ht_recycle_bin/js/loadingoverlay.min.js"}'></script>
<script type='text/javascript' src='{sugar_getjspath file="modules/ht_recycle_bin/js/show_popup.js"}'></script>
...
{if count($data) == 0}
    ...
    {if $displayEmptyDataMesssages}
        {if strlen($query) == 0}
                ...
                <p class="msg">
                    There are no deleted records for this module.
                </p>
	...
	{/if}
		...
		{if $showFilterIcon}
			{include file='modules/ht_recycle_bin/ListView/tpls/ListViewSearchLink.tpl'}
		{/if}
	</div>
{/if}
{$multiSelectData}
{if $hideTable == false}
	...
	{include file='modules/ht_recycle_bin/ListView/tpls/ListViewPagination.tpl'}
	<tr height='20'>
			{if $prerow}
			...
			<tr height='20' class='{$_rowColor}S1'>
				...
				<td width='2%' nowrap>
                    {if $pageData.rowAccess[$id].edit}
                        ...
                        <a title='{$editLinkString}' id="edit-{$rowData.ID}"
                           href="index.php?module=ht_recycle_bin&offset={$offset}&stamp={$pageData.stamp}&return_module={$linkModule}&action={$action}&record={$rowData.ID}"
                                >
				...
				{foreach from=$displayColumns key=col item=params}
					...
					<td {if $scope_row} scope='row' {/if} align='{$params.align|default:'left'}' valign="top" type="{$displayColumns.$col.type}" field="{$col|lower}" class="{$hiddenclass} {if $inline_edit && ($displayColumns.$col.inline_edit == 1 || !isset($displayColumns.$col.inline_edit))}inlineEdit{/if}{if ($params.type == 'teamset')}nowrap{/if}{if preg_match('/PHONE/', $col)} phone{/if}">
					    ...
					    {if $col == 'NAME' || $col == 'FULL_NAME' && $params.link && !$params.customCode}
	                        ...
	                        <{$pageData.tag.$id[$params.ACLTag]|default:$pageData.tag.$id.MAIN} onclick="show_reocrd_detail('{$linkModule}', '{$record}')" >
						{/if}
