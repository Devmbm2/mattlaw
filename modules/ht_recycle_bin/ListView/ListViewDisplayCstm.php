...
require_once('modules/ht_recycle_bin/ListView/ListViewDataCstm.php');
require_once('include/MassUpdate.php');

class ListViewDisplayCstm {
	...
	public function display()
	{
		...
		if ($this->show_mass_update_form) {
			$str = $this->mass->getDisplayMassUpdateForm(true, $this->multi_select_popup).$this->getMassUpdateFormHeader($this->multi_select_popup);
		}
    ...
    function getMassUpdateFormHeader($multi_select_popup = false)
    {
        ...
        $bean = loadBean($this->seed->module_dir);
...
eoq;
	...
	function buildSelectedObjectsSpan($echo = true, $total=0) {
		...
		$selectedObjectSpan = "<span style='$displayStyle'>{$app_strings['LBL_LISTVIEW_SELECTED_OBJECTS']}<input  style='border: 0px; background: transparent; font-size: inherit; color: inherit' type='text' id='selectCountTop' readonly name='selectCount[]' value='{$total}' /></span>";
