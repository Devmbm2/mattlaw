...
//								 "ProjectTask" => array("showCompleted" => true,"start" =>  "date_start", "end" => "date_finish"),
	...
	public $shared_ids_colors = array(); // colors of users for shared view
	...
	function __construct($view = "agendaWeek", $time_arr = array()){
		...
		$this->show_events = $current_user->getPreference('show_events');
		if(is_null($this->show_events))
			$this->show_events = SugarConfig::getInstance()->get('calendar.show_events_by_default',true);
	...
	public function load_activities(){
		...
		foreach($this->acts_arr as $user_id => $acts){
			if(isset($acts) && empty($acts)){
				$shared_calendar_separate = SugarConfig::getInstance()->get('calendar.calendar_display_shared_separate', false);
			...
			foreach($acts as $act){
					...
					$multiple_Users = unencodeMultienum($act->sugar_bean->multiple_assigned_users);
					if($multiple_Users){
						foreach($multiple_Users AS $user_id){
							$userBean = BeanFactory::getBean('Employees', $user_id);
							if($userBean){
								$item['name'] .= strtoupper($userBean->initials_c) . ' - ';
							}
						}
					...
					if($act->sugar_bean->module_dir == "FP_events"){
						$FP_events = BeanFactory::getBean('FP_events', $act->sugar_bean->id);
						$item['cancelled_reset_c'] = $FP_events->cancelled_reset_c;
						if($FP_events->cancelled_reset_c == 'Cancelled'){
							$item['name'] = $FP_events->cancelled_reset_c .' - '. $item['name'];
						}
					}
				...
				if($shared_calendar_separate){
					...
					$this->items[ $GLOBALS['current_user']->id ][$item['id']] = $item;
				}
			}
			$i++;
		}
		// print"<pre>";print_r($this->items);die;
	}	
	...
	public function init_shared(){
		...
		$shared_ids_colors = $current_user->getPreference('shared_ids_colors');
		if(!empty($user_ids) && count($user_ids) != 0 && !isset($_REQUEST['shared_ids'])) {
			$this->shared_ids = $user_ids;
			$this->shared_ids_colors = $shared_ids_colors;
		...
		if(isset($_REQUEST['shared_ids_colors']) && count($_REQUEST['shared_ids_colors']) > 0){
			$this->shared_ids_colors = $_REQUEST['shared_ids_colors'];
			$current_user->setPreference('shared_ids_colors', $_REQUEST['shared_ids_colors']);
		}
	}
	...
	public function add_activities($user,$type='sugar'){
		...
		$acts_arr = array();
	    	...
	    	{
				$acts_arr = CalendarActivity::get_activities($this->activityList, $user->id, $this->show_tasks, $start_date_time, $end_date_time, $this->view, $this->show_calls, $this->show_events, $this->show_completed);
	    	}
