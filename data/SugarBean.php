...
class SugarBean
{
                                                  ...
                                                  $row_offset = 0, $limit = -1, $max = -1, $show_deleted = 0, $subpanel_def = null)
    {
        ...
        foreach ($subpanel_list as $this_subpanel) {
			$related_modules[] = $this_subpanel->_instance_properties['module'];
        ...
        $response = $parentbean->process_union_list_query($parentbean, $final_query, $row_offset, $limit, $max, '', $subpanel_def, $final_query_rows, $secondary_queries);
		if($response['row_count'] > 0){
			foreach ($related_modules as $related_module) {
				SugarVCR::erase($related_module);
				SugarVCR::store($related_module, $final_query);
				SugarVCR::recordIDs($related_module, array_keys($response['list']), $response['current_offset'], $response['row_count']);
			}
		}
		return  $response;
    }
    ...
    public function getRelatedFields($module, $id, $fields, $return_array = false)
    {
        ...
        foreach ($fields as $field => $alias) {
            if (!empty($GLOBALS['dictionary'][$object]['fields'][$field]['db_concat_fields'])) {
				$separator = !empty($GLOBALS['dictionary'][$object]['fields'][$field]['db_fields_separator']) ? $GLOBALS['dictionary'][$object]['fields'][$field]['db_fields_separator'] : ' ';
                $query .= ' ,' . $this->db->concat($table, $GLOBALS['dictionary'][$object]['fields'][$field]['db_concat_fields'], $separator) . ' as ' . $alias;
